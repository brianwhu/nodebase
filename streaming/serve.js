...
export default toplevel => {
    ...
    if (cli) {
        ...
        app.get("/video", async function (req, res) {
            ...
            if (videoPath.endsWith("mpx")) {
                const { default: Transcoder } = await import("@streamyflow/mpx-transcoder")
        ...
        if (cli.opts.open.value) {
            import("@streamyflow/chrome-window").then(module => {
                module.default.open(`http://localhost:${cli.opts.port.value}/`)
            })
        }
    }
}
